...
export async function fetchEpisodes(page: number = 1): Promise<[{id: number, name: string}]> {
  ...
  return data.results?.length ? data.results.map((ep:any) => ({id: ep.id, name: ep.name})):[];
}
...
export async function fetchEpisodeWithCharacters(id: number ) {
  ...
  const data = await response.json();
  const characters = data.characters?.length? Promise.all(data.characters?.map(fetchCharacters)): []
